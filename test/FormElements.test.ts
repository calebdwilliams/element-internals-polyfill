...
async function createForm(): Promise<HTMLFormElement> {
  return await fixture<HTMLFormElement>(html`
    <form id="form">
      ...
      <md-radio name="radio" id="r1"></md-radio>
...
it('must contains the custom elements associated to the current form, in the correct order', async () => {
  ...
  expect(form.elements[4]).to.be.an.instanceof(MdRadio);
  expect(form.elements[5]).to.be.an.instanceof(HTMLButtonElement);
  ...
  expect(form.elements[4].id).to.equal('r1');
  ...
  expect(form.elements.namedItem('radio')).to.be.an.instanceof(MdRadio);
});
